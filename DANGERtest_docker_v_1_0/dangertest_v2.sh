...
mm_fn="/DATA/${5}" # "Result_offtarget_mm"$i".cas-offinder"
analysis_dir="/DATA/${6}" # "park7_result_original"
ontarget_fasta="/DATA/${7}" # "/DATA/guide_pam.fa"
thread=16
...
for i in {0..11};
  do awk -v MM=$i '{ if ($6 == MM) { print } }' ${mm_fn} > ${outpur_dir}/Result_offtarget_mm"$i".cas-offinder;
...
awk '{ print $2 }' ${mm_fn} > ${outpur_dir}/Result_offtarget_all_list.txt;
