...
total_bp_file_size = 0
for table_fn in table_fn_list:
  if os.path.isfile(join(table_fn, "BP_enrichment_table.txt")):
    total_bp_file_size = total_bp_file_size + os.path.getsize(join(table_fn, "BP_enrichment_table.txt"))

total_cc_file_size = 0
for table_fn in table_fn_list:
  if os.path.isfile(join(table_fn, "CC_enrichment_table.txt")):
    total_cc_file_size = total_cc_file_size + os.path.getsize(join(table_fn, "CC_enrichment_table.txt"))

total_mf_file_size = 0
for table_fn in table_fn_list:
  if os.path.isfile(join(table_fn, "MF_enrichment_table.txt")):
    total_mf_file_size = total_mf_file_size + os.path.getsize(join(table_fn, "MF_enrichment_table.txt"))

if (total_bp_file_size == 0) or (total_cc_file_size == 0) or (total_mf_file_size == 0):
  print("There is no sufficient data for D-index calculation.")
  print("Try it with more mismatch numbers to ensure data.")
  sys.exit(0)
...
def mergeEnrichmentTable(input_fp, table_fn_list):
  ...
  for table_fn in table_fn_list:
    if os.path.isfile(join(table_fn, input_fp)):
